...
def test_import_skills_from_github_local_path(tmp_path: Path):
    vault = _make_vault(tmp_path)

    repo_dir = tmp_path / "awesome-codex-skills"
    repo_dir.mkdir(parents=True, exist_ok=True)
    (repo_dir / "skill_repo.md").write_text(
        "# Repo Skill\n\n"
        "Skill imported from repository source.\n\n"
        "- Discover files\n"
        "- Build execution plan\n",
        encoding="utf-8",
    )

    result = vault.import_skills_from_github(
        repo_url=str(repo_dir),
        source="awesome-codex-skills",
        default_tags=["github", "imported"],
    )

    assert result["sync"]["mode"] == "local-path"
