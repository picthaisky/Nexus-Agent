...
2. Import skills from local markdown directory.
...
3. Import skills directly from GitHub (clone or pull).
...
4. Search and suggest reusable skills.
...
5. Record execution outcomes for maturity evolution.
...
6. Generate deep research and autonomous task plan.
...
1. Build graph.
2. Trace execution path from target entrypoint.
3. Run blast radius for proposed symbol changes.
4. Prepare refactor plan in dry-run mode.
5. Apply only after validation checkpoints.
6. Record outcomes in Skill Vault for future automation.
